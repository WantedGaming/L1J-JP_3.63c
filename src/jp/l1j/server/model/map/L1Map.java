...
public abstract class L1Map implements Cloneable {
	...
	public L1Map clone() {
		try {
			...
			// ありえない
			throw new InternalError("CloneNotSupportedException was thrown");
		}
	}

	/**
	 * マップIDを返します。 <b>このIDをゲームクライアントに送信しないでください。</b>
	 * このIDはマップオブジェクトの一意に識別するために利用される値であり、マップファイルIDと同一であることは保証されません。
	 * マップファイルIDが必要な場合は、{@link #getBaseMapId()}を利用してください。
	 * 
	 * @see #getBaseMapId()
	 * @return マップID
	 */
	...
	/**
	 * 新しいマップIDを設定します。 このIDはマップオブジェクトを一意に識別できる値であることが保証されなければなりません。
	 * 
	 * @param mapId
	 *            新しく設定されるマップID。
	 */
	...
	/**
	 * マップファイルIDを返します。 <b>このIDをマップオブジェクトの一意に識別するために利用しないでください。</b>
	 * このIDはマップファイルを識別するためにゲームクライアントが必要とするIDであり、一意性は保証されません。
	 * いくつかのオブジェクトが同じマップファイルIDを持つことがあります。
	 * クライアントに送信するマップファイルIDが必要な場合に、このメソッドを利用します。
	 * 
	 * @return マップファイルID
	 */
