...
public class L1TownLocation {
	private static Logger _log = Logger.getLogger(L1TownLocation.class
			.getName());
	...
	public static int getTownTaxRateByNpcid(int npcid) { // npcidから町税率を返す
		...
		if (town_id >= 1 && town_id <= 10) {
			...
			tax_rate = town.getTaxRate() + 2; // 2%は固定税
		}
		return tax_rate;
	}
	...
	public static int getTownIdByNpcid(int npcid) { // npcidからtown_idを返す
		...
		case 80259: // ピーター（象牙の塔テレポーター）
			town_id = TOWNID_OREN;
			break;

		case 50044: // シリウス（テレポーター）
		case 50046: // エレリス（テレポーター）
		...
		case 50072: // ディアルズ（テレポーター）
		case 50079: // ダニエル（テレポーター）
		case 70092: // 商人 エマルト
		case 70093: // 商人 カルプ
